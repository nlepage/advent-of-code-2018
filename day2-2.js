...
outer: for (let i = 0; i < boxes[0].length; i++) {
    const fixedBoxes = boxes.map(box => box.slice(0, i) + box.slice(i + 1))
    for (let j = 0; j < fixedBoxes.length; j++) {
        if (fixedBoxes.indexOf(fixedBoxes[j], j + 1) !== -1) {
            console.log(fixedBoxes[j])
        }
    }
}